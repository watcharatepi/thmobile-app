			...
			$.ajax({
				...
				'success' : function(result) {
					...
					if ( typeof result.responses[0].errorCode !== "undefined") {
						...
						var link = "http://mobile.unicity-th.com/payment-bkk/?data=" + encodeURIComponent(order_data)
...
cartPageCtrl.period_on = false;
	...
	function($http, element, $scope, $rootScope, $filter, cart, sharedComponents) {
		...
		shoppingPageCtrl.getCategories = function() {
			...
			shoppingPageCtrl.categories = [];
			$http.get('http://mobile.unicity-th.com/wp-admin/admin-ajax.php?action=unimobiapp_get_product_categories').success(function(response) {
				shoppingPageCtrl.categories = [];
		...
		$scope.$watch(function() {
			...
			if (v != "shopping") {
				...
				shoppingPageCtrl.changeTab("product-easyship");
...
var obj  = jQuery.parseJSON(dataSet);
	...
	localStorage.setItem("perid_on",obj.perid);
		...
		shoppingPageCtrl.loadData = function(page) {
			...
			$.ajax({
				'type' : 'GET',
				'url' : 'http://mobile.unicity-th.com/wp-admin/admin-ajax.php?action=unimobiapp_get_products&args[order]=asc&args[posts_per_page]=10&args[paged]=100',
				'dataType' : 'jsonp',
				'success' : function(result) {
					console.log(" result=====> "+result);
			...
			//	$('#page-shopping').addClass("loading");
			$http.get('http://mobile.unicity-th.com/wp-admin/admin-ajax.php?action=unimobiapp_get_products&args[order]=asc&args[posts_per_page]=10&args[paged]=' + page).success(function(response) {
